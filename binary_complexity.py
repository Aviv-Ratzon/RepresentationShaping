...
s_bits = 4
a_bits = 4
s_values = np.round(np.linspace(-.5, .5, 2**s_bits), 4)       # 4 values
a_values = np.round(np.linspace(-.5, .5, 2**a_bits), 4)   # 4 values
...
s_in = np.random.randn(len(s_values), 2)
a_in = np.random.randn(len(a_values), 2)
unique_sorted_s = sorted(set(s.squeeze()))
s_inds = [unique_sorted_s.index(x) for x in s.squeeze()]
unique_sorted_a = sorted(set(a.squeeze()))
a_inds = [unique_sorted_a.index(x) for x in a.squeeze()]
s_in = s_in[s_inds]
a_in = a_in[a_inds]
x_continuous = np.concatenate([s_in, a_in], axis=1)


# [plt.plot(s, x_continuous[:,i], marker='o') for i in range(5)]
# plt.show()
# [plt.plot(a, x_continuous[:,5+i], marker='o') for i in range(5)]
# plt.show()
...
y_task = s+a # generate_twisted_polynomial_vector(s.squeeze()+a.squeeze(), input_dim=1).squeeze()
# plt.scatter((s+a).squeeze(), y_task, marker='o')
# plt.show()
