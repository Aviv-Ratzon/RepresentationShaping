...
for i_seed, seed in tqdm(enumerate(range(n_seeds))):
    for i_max_move, max_move in enumerate(max_move_l):
        ...
        r2_score, pr_score = model.plot_pca_last_layer(
            ...
            save_path=f"last_layer_pca.png",
            title="PCA",
            show=False
        )
        r2_l[i_max_move, i_seed] = r2_score
        pr_l[i_max_move, i_seed] = pr_score
        train_score_l[i_max_move, i_seed] = model.score(X, y)
        ...
        with open("result_arrays.pkl", "wb") as f:
